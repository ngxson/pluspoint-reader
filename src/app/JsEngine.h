...
// Compiled JS code format
typedef struct {
	uint8_t *buf;
	size_t len;
} js_compiled_t;

// Compile JS source to dense binary format
js_compiled_t js_compile(const char *src, size_t len);

struct js *js_create(void *buf, size_t len);         // Create JS instance
// Execute JS code (source or compiled)
jsval_t js_eval(struct js *, const char *src, size_t len);  // Execute JS source
jsval_t js_eval_compiled(struct js *, const js_compiled_t *compiled); // Execute compiled code
...
const char *js_errmsg(struct js *js);                          // Get last error

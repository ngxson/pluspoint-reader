...
#define F_COMPILED 32U // Using compiled code
  ...
  const js_compiled_t *compiled; // Compiled code buffer (when F_COMPILED is set)
};
...
enum { 
  ...
  // Compiled code format IDs
  TOK_COMPILED_STR = 200, TOK_COMPILED_NUM, TOK_COMPILED_KW, TOK_COMPILED_END
};
// Compiled format IDs (must match JsCompile.c)
#define COMPILED_STR  0x01
#define COMPILED_NUM  0x02
#define COMPILED_KW   0x03
#define COMPILED_IDENT 0x04
#define COMPILED_END  0xFF
...
static inline jsval_t js_mktrue(void) { return mkval(T_BOOL, 1); }
static inline jsval_t js_mkfalse(void) { return mkval(T_BOOL, 0); }
static inline jsval_t js_mkundef(void) { return mkval(T_UNDEF, 0); }
static inline jsval_t js_mknull(void) { return mkval(T_NULL, 0); }
...
static uint8_t next(struct js *js) {
  ...
  // Handle compiled code mode
  if (js->flags & F_COMPILED) {
    if (js->pos >= js->compiled->len) {
      js->tok = TOK_EOF;
      return js->tok;
    }

    uint8_t id = js->compiled->buf[js->pos++];

    if (id == COMPILED_END) {
      js->tok = TOK_EOF;
      return js->tok;
    } else if (id == COMPILED_STR) {
      // String literal
      js->tok = TOK_STRING;
      js->tlen = js->compiled->buf[js->pos++];
      js->toff = js->pos;
      // Create string value
      js->tval = js_mkstr(js, &js->compiled->buf[js->pos], js->tlen);
      js->pos += js->tlen;
      return js->tok;
    } else if (id == COMPILED_NUM) {
      // Number literal
      js->tok = TOK_NUMBER;
      js->tlen = js->compiled->buf[js->pos++];
      // Convert string representation to number
      char numbuf[32];
      if (js->tlen < sizeof(numbuf)) {
        memcpy(numbuf, &js->compiled->buf[js->pos], js->tlen);
        numbuf[js->tlen] = '\0';
        js->tval = tov(strtod(numbuf, NULL));
      }
      js->pos += js->tlen;
      return js->tok;
    } else if (id == COMPILED_KW) {
      // Keyword
      js->tok = js->compiled->buf[js->pos++];
      js->tlen = 0;
      return js->tok;
    } else if (id == COMPILED_IDENT) {
      // Identifier
      ...
      // Direct token (operator, punctuation)
      js->tok = id;
      js->tlen = 0;
      return js->tok;
    }
...
// Get pointer to current token buffer (handles both source and compiled mode)
static const char *get_tok_ptr(struct js *js) {
  if (js->flags & F_COMPILED) {
    return (const char *)(js->compiled->buf + js->toff);
  }
  return js->code + js->toff;
}

// Get pointer for coderef (handles both source and compiled mode)
static const char *get_coderef_ptr(struct js *js, jsval_t coderef) {
  jsoff_t off = coderefoff(coderef);
  if (js->flags & F_COMPILED) {
    return (const char *)(js->compiled->buf + off);
  }
  return js->code + off;
}

// Get character at current position (handles both source and compiled mode)
static char get_char_at_pos(struct js *js, jsoff_t pos) {
  if (js->flags & F_COMPILED) {
    return (pos < js->compiled->len) ? (char)js->compiled->buf[pos] : '\0';
  }
  return (pos < js->clen) ? js->code[pos] : '\0';
}

// Skip to next token, handling both modes
static jsoff_t skip_to_next(struct js *js, jsoff_t pos) {
  if (js->flags & F_COMPILED) {
    // In compiled mode, tokens are already dense, just return current pos
    return pos;
  }
  return skiptonext(js->code, js->clen, pos);
}
...
static jsval_t do_dot_op(struct js *js, jsval_t l, jsval_t r) {
  ...
  const char *ptr = get_coderef_ptr(js, r);
...
static jsval_t call_js(struct js *js, const char *fn, jsoff_t fnlen) {
  ...
  while (fnpos < fnlen) {
    ...
    //       (int) js->clen, js->code);
    js->pos = skip_to_next(js, js->pos);
    js->consumed = 1;
    jsval_t v = get_char_at_pos(js, js->pos) == ')' ? js_mkundef() : js_expr(js);
    // Set argument in the function scope
    setprop(js, js->scope, js_mkstr(js, &fn[fnpos], identlen), v);
    js->pos = skip_to_next(js, js->pos);
    if (get_char_at_pos(js, js->pos) == ',') js->pos++;
...
static jsval_t do_call_op(struct js *js, jsval_t func, jsval_t args) {
  ...
  // Set up to parse function arguments
  if (js->flags & F_COMPILED) {
    // In compiled mode, adjust position within compiled buffer
    js->pos = coderefoff(args);
    js->clen = coderefoff(args) + codereflen(args);
  } else {
    // In source mode, point to args substring
...
static jsval_t js_str_literal(struct js *js) {
  // In compiled mode, string is already processed and stored in tval
  if (js->flags & F_COMPILED) {
    return js->tval;
  }
...
static jsval_t js_obj_literal(struct js *js) {
  ...
  while (next(js) != TOK_RBRACE) {
    ...
    if (js->tok == TOK_IDENTIFIER) {
      if (exe) key = js_mkstr(js, get_tok_ptr(js), js->tlen);
...
static jsval_t js_call_dot(struct js *js) {
  ...
  if (vtype(res) == T_CODEREF) {
    res = lookup(js, get_coderef_ptr(js, res), codereflen(res));
  }
...
static jsval_t js_stmt(struct js *js) {
  ...
  switch (next(js)) {  // clang-format off
    ...
    case TOK_WITH: case TOK_WHILE: case TOK_YIELD:
      res = js_mkerr(js, "'%.*s' not implemented", (int) js->tlen, get_tok_ptr(js));
      break;
...
// Convenience functions moved to top of file as static inline
...
// Evaluate compiled JS code
jsval_t js_eval_compiled(struct js *js, const js_compiled_t *compiled) {
  jsval_t res = js_mkundef();
  if (!compiled || !compiled->buf || compiled->len == 0) {
    return js_mkerr(js, "invalid compiled code");
  }

  // Save current state
  uint8_t old_flags = js->flags;
  const char *old_code = js->code;
  jsoff_t old_clen = js->clen;
  jsoff_t old_pos = js->pos;

  // Set up compiled mode
  js->flags |= F_COMPILED;

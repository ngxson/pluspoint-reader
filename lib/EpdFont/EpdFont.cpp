...
void EpdFont::getTextBounds(const char *string, const int startX,
  ...
  *maxY = startY;
  ...
  int lastBaseWidth = 0;
  int lastBaseTop = 0;
  int32_t prevAdvanceFP =
      0; // 12.4 fixed-point: prev glyph's advance + next kern for snap
  uint32_t cp;
  uint32_t prevCp = 0;
  while (
    ...
    if (!glyph) {
      // Keep cursor movement stable when a base glyph is missing, but don't
      // attach subsequent combining marks to stale base metrics.
      if (!isCombining) {
        lastBaseX += fp4::toPixel(
            prevAdvanceFP); // flush pending advance before resetting
        prevCp = 0;
        prevAdvanceFP = 0;
        lastBaseLeft = 0;
        lastBaseWidth = 0;
    ...
    const int raiseBy =
        isCombining ? combiningMark::raiseAboveBase(glyph->top, glyph->height,
                                                    lastBaseTop)
                    : 0;
    ...
    const int glyphBaseX =
        isCombining
            ? combiningMark::centerOver(lastBaseX, lastBaseLeft, lastBaseWidth,
...
static uint8_t lookupKernClass(const EpdKernClassEntry *entries,
                               const uint16_t count, const uint32_t cp) {
  ...
  // lower_bound: exact-key lookup. Finds the first entry with codepoint >=
  // target, then the equality check confirms an exact match exists.
  const auto it = std::lower_bound(
      entries, end, target, [](const EpdKernClassEntry &entry, uint16_t value) {
...
int8_t EpdFont::getKerning(const uint32_t leftCp,
                           const uint32_t rightCp) const {
  if (!data->kernMatrix) {
    return 0;
  }
  const uint8_t lc =
  ...
  if (lc == 0)
    return 0;
  const uint8_t rc = lookupKernClass(data->kernRightClasses,
...
uint32_t EpdFont::getLigature(const uint32_t leftCp,
                              const uint32_t rightCp) const {
  ...
  const auto it = std::lower_bound(
      pairs, end, key, [](const EpdLigaturePair &pair, uint32_t value) {
...
uint32_t EpdFont::applyLigatures(uint32_t cp, const char *&text) const {
  ...
  while (true) {
    ...
    const uint32_t nextCp =
        utf8NextCodepoint(reinterpret_cast<const uint8_t **>(&text));
    if (nextCp == 0)
      break;
...
const EpdGlyph *EpdFont::getGlyph(const uint32_t cp) const {
  ...
  if (count > 0) {
    ...
    // upper_bound: range lookup. Finds the first interval with first > cp, so
    // the interval just before it is the last one with first <= cp. That's the
    // only candidate that could contain cp. Then we verify cp <=
    // candidate.last.

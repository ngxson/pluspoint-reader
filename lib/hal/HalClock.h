...
public:
  ...
  // 24h mode produces "HH:MM" (needs >=6 bytes); 12h mode produces "H:MM
  // AM"/"HH:MM PM" (needs >=9 bytes). utcOffsetQuarterHoursBiased: biased
  // quarter-hour offset (48 = UTC+0, 0 = UTC-12, 104 = UTC+14). use12Hour: when
  // true, format as 12-hour clock with AM/PM suffix. Returns false if RTC is
  // not available.
